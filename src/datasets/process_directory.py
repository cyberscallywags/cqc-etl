"""Process CQC directory data"""

# Imports
import json
import pandas as pd
from src.utils.parsers import decompose_list, clean_document
from src.utils.transforms import combine_csvs
...
r_df.columns = ["name"]
r_df = r_df.dropna().sort_values(by="name").reset_index(drop=True)
r_df.to_json(PROCESSED_DATA_DIR / "regions.json", indent=2, orient='records')
...
la_df = la_df.sort_values(by=["region", "name"]).reset_index(drop=True)
la_df.to_json(PROCESSED_DATA_DIR / "local_authorities.json", indent=2, orient='records')

# Create postcodes JSON
pc_df = df[["postcode", "local_authority", "region"]].copy().drop_duplicates()
pc_df = pc_df.dropna(subset=["postcode"])
pc_df = pc_df.sort_values(by=["region", "local_authority"]).reset_index(drop=True)
pc_df.to_json(PROCESSED_DATA_DIR / "postcodes.json", indent=2, orient='records')

# Create providers JSON
p_df = df[["provider", "provider_id"]].copy().drop_duplicates()
p_df.columns = ["name", "id"]
p_df = p_df.dropna(subset=["name"])
p_df = p_df.sort_values(by=["name"]).reset_index(drop=True)
p_df.to_json(PROCESSED_DATA_DIR / "providers.json", indent=2, orient='records')

# Create services JSON
s_df = pd.DataFrame(decompose_list(list(df["services"].unique()), "|"), columns=["name"])
s_df = s_df.sort_values(by="name").reset_index(drop=True)
s_df.to_json(PROCESSED_DATA_DIR / "services.json", indent=2, orient='records')

# Create service types JSON
st_df = pd.DataFrame(decompose_list(list(df["service_types"].unique()), "|"), columns=["name"])
st_df = st_df.sort_values(by="name").reset_index(drop=True)
st_df.to_json(PROCESSED_DATA_DIR / "service_types.json", indent=2, orient='records')
...
for location in locations:
    r_location = {
        ...
        "url": location["url"],
        "services": list(set([i.strip() for i in str(location["services"]).split("|")])),
        "service_types": list(set([i.strip() for i in location["service_types"].split("|")])),
